...
HEADERS = {
    "User-Agent": "KuwagataTVProject/2.4 (private educational production; contact via GitHub repository)",
...
def download_one(asset: dict[str,str]) -> dict[str,object]:
    ...
    url=original_url(asset['filename']); last=None
    for attempt in range(5):
        try:
            print(f"GET {asset['key']} attempt={attempt+1} {url}",flush=True)
            tmp=target.with_suffix(target.suffix+'.part')
            with requests.get(url,headers=HEADERS,timeout=(30,300),stream=True,allow_redirects=True) as response:
                ...
                if asset['kind']=='video' and not any(x in ctype for x in ('video','ogg','octet-stream')):
                    raise RuntimeError(f'unexpected content type {ctype}')
                with tmp.open('wb') as fh:
                    for chunk in response.iter_content(1024*1024):
                        if chunk: fh.write(chunk)
            if tmp.stat().st_size<1000: raise RuntimeError(f'download too small {tmp.stat().st_size}')
            tmp.replace(target)
            print('OK',asset['key'],target.stat().st_size,flush=True)
            return {'key':asset['key'],'status':'downloaded','bytes':target.stat().st_size,'download_url':url}
        except Exception as exc:
            last=exc; print('RETRY',asset['key'],repr(exc),flush=True); time.sleep(5+attempt*5)
    return {'key':asset['key'],'status':'failed','error':repr(last)}


def validate_asset(asset: dict[str,str]) -> dict[str,object]:
    path=OUT/asset['local']; row={**asset,'bytes':path.stat().st_size}
    if asset['kind']=='image':
        with Image.open(path) as im: im.load(); row['width'],row['height']=im.size
    else:
...
def make_montage() -> None:
    ...
    for fp in ['/usr/share/fonts/opentype/noto/NotoSansCJK-Bold.ttc','/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc']:
        if Path(fp).exists(): fnt=ImageFont.truetype(fp,28); break
    thumbs=[]
    for asset in ASSETS:
        path=OUT/asset['local']
        if asset['kind']=='image': im=Image.open(path).convert('RGB')
        else:
    ...
    canvas=Image.new('RGB',(cols*cell_w,rows*cell_h),'white'); d=ImageDraw.Draw(canvas)
    for i,(label,im) in enumerate(thumbs):
        x=(i%cols)*cell_w+(cell_w-im.width)//2; y=(i//cols)*cell_h
        canvas.paste(im,(x,y)); d.text(((i%cols)*cell_w+15,y+330),label,font=fnt,fill='black')
    canvas.save(OUT/'asset_montage.jpg',quality=92)


def main() -> None:
    # Two concurrent transfers avoid Commons throttling while keeping total asset time reasonable.
    ...
    if failures: raise RuntimeError(f'Asset downloads failed: {failures}')
